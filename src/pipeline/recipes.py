...
def ingest_md_to_json(
    ...
    output = {"recipes": all_recipes, "restaurant": all_restaurants}
...
#     output_full_info = []
#     print(f"Processing {len(dict_list)} {key_text}")
#     for recipe in tqdm(dict_list):
#         recipe_text = dict_list[key_text]
...
#             output_full_info.append(recipe)
#         except Exception as e:
#             output_full_info.append(
#                 {
#                     **recipe,
...
def add_planets_and_order(restaurant: List[Dict]) -> List[Dict]:
    """
    Adds fake ingredients and techniques to each restaurant.
    Args:
        restaurant (list): List of restaurant dictionaries.
    Returns:
        list: List of restaurant dictionaries with added ingredients and techniques.
    """
    provider = Config.provider
    model = Config.model
    output_model_str = get_model_source("src.datamodels", "Restaurant")

    output_restaurants_full_info = []
    print(f"Processing {len(restaurant)} restaurant")
    for restaurant in tqdm(restaurant):
        restaurant_text = restaurant["restaurant_text"]
        system_message = Config.system_message_template_restaurant_recipe.format(
            output_model_str=output_model_str
        )
        message = Config.system_message_template_restaurant_recipe.format(
            request=restaurant_text
        )
        ...
        try:
            ...
            output_restaurants_full_info.append(restaurant)
        except Exception as e:
            output_restaurants_full_info.append(
                {
                    **restaurant,
                    "planet_info": f"Error: {e}",
                    "order_info": "Error",
                }
            )

    return output_restaurants_full_info


def process_answer_pipeline(
    input_path: Path | str,
    recipes_output_path: Path | str,
    restaurant_output_path: Path | str,
    to_answer_output: Path | str,
) -> List[Dict]:
    """
    ...
    recipes_output_path = Path(recipes_output_path)
    restaurant_output_path = Path(restaurant_output_path)
    to_answer_output = Path(restaurant_output_path)

    if recipes_output_path.exists():
        with recipes_output_path.open("r") as f:
            all_recipes = json.load(f)
    else:
        all_recipes = ingest_md_to_json(
    ...
    if restaurant_output_path.exists():
        ...
        all_restaurants = ingest_md_to_json(
            input_path, recipes_output_path, restaurant_output_path
        )["restaurant"]
        all_restaurants = add_planets_and_order(all_restaurants)

        with restaurant_output_path.open("w") as f:
            json.dump(all_restaurants, f, indent=4)

    if to_answer_output.exists():
        with to_answer_output.open("r") as f:
            to_answer = json.load(f)
    else:
        to_answer = []
        recipes_restaurants = {
            recipe["recipes_restaurant"]: recipe for recipe in all_recipes
        }

        for restaurant in all_restaurants:
            restaurant_name_tmp = restaurant["restaurant_name"]
        if restaurant_name_tmp in recipes_restaurants:
            join_recipe_restaurant = {
                **restaurant,
                **recipes_restaurants[restaurant_name_tmp],
            }
            to_answer.append(join_recipe_restaurant)
        else:
            to_answer.append(restaurant)

        with to_answer_output.open("w") as f:
            json.dump(to_answer_output, f, indent=4)
